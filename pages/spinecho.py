...
from qick_workspace.scrip.s007_SpinEcho_ge import SpinEchoProgram
...
class SpinEchoController(BaseMeasurementController):
    """Encapsulates the measurement and plotting logic for the Spin Echo page."""

    def __init__(self, app_state: 'AppState', spinecho_state: SpinEchoState):
        super().__init__(app_state, spinecho_state)
        ...
        try:
            with self.fit_plot_container:
                with ui.matplotlib(figsize=(12, 6)).figure as fig:
                    ...
                    title = f"Spin Echo Fit (Time: {timestamp})"
                    ...
                    if self.state.ramsey_freq != 0:
                        ...
                        # T2 = fit_params[3]
                        ui.label(f"T2 Echo: {fit_params[3]:.4f} us, Detuning: {fit_params[1]:.4f} MHz").classes("text-lg font-bold")
                    else:
                        fit_params, error, _ = nicegui_plot_final(
                            times, 
                            iq_data, 
                        ...
                        # T2 = fit_params[2]
                        ui.label(f"T2 Echo: {fit_params[2]:.4f} us").classes("text-lg font-bold")
    ...
    async def run_measurement(self):
        ...
        try:
            ...
            # Total time is wait1 + wait2
            times1 = prog.get_time_param("wait1", "t", as_array=True)
            times2 = prog.get_time_param("wait2", "t", as_array=True)
            times = times1 + times2
        ...
        with self.plot_container:
            ...
            with fig_element.figure as fig:
                ...
                ax.set_ylabel("|IQ|")
                ax.set_title("Spin Echo Result (Initializing...)")
        ...
        def plot_callback(data: np.ndarray, avg_count: int):
            ...
            ax.set_title(f"Spin Echo Result (Avg: {avg_count})")
            fig_element.update()
...
def add_page(app_state):
    create_measurement_page(
        page_route="/spinecho",
        page_title="Spin Echo Measurement",
        controller_class=SpinEchoController,
        app_state=app_state,
        state_attr="spinecho_state",
        settings_card_func=wait_settings_card,
        settings_card_kwargs={},
        plot_title="Spin Echo Result",
        fit_plot_title="Spin Echo Fit",
